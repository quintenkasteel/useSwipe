...
const useSwipe = (element, preventDefault, callback = () => {}) => {
  ...
  const handleStart = useCallback(event => {
    ...
    return swipe;
  }, [swipe]); // handle the end of the swipe

  const handleEnd = useCallback(event => {
    event.preventDefault();
    const touch = event.changedTouches[0];
    const direction = touch.pageX - swipe.startX > 20 && 'RIGHT' || swipe.startX - touch.pageX > 20 && 'LEFT' || touch.pageY - swipe.startY > 20 && 'DOWN' || swipe.startY - touch.pageY > 20 && 'UP';
    setSwipe({ ...swipe,
      endX: touch.pageX,
      endY: touch.pageY,
    ...
    return swipe;
  }, [swipe]); // handle the cancellation of the swipe
